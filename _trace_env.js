// 探索：记录 bdms SDK 签名时访问的环境属性（基于已验证可用的完整 shim）
"use strict";
const fs = require("fs");
...
function trap(label, obj) {
  return new Proxy(obj, {
    get(target, prop, receiver) {
      if (typeof prop === "string") accessed.add(label + "." + prop);
      const v = Reflect.get(target, prop, receiver);
      return v;
    },
    set(target, prop, value, receiver) {
      if (typeof prop === "string") accessed.add(label + "." + prop + "(set)");
...
function makeLocation() {
  const loc = {
    href: "https://www.shidianguji.com/", origin: "https://www.shidianguji.com", protocol: "https:",
    host: "www.shidianguji.com", hostname: "www.shidianguji.com", port: "", pathname: "/", search: "", hash: "",
    ancestorOrigins: {}, assign: fakeFn, replace: fakeFn, reload: fakeFn,
    toString: () => loc.href,
  };
  return loc;
}
...
function makeNavigator() {
  const nav = {
    userAgent: UA, platform: "Win32", language: "zh-CN", languages: ["zh-CN", "zh", "en"],
    vendor: "Google Inc.", appVersion: "5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36",
    cookieEnabled: true, onLine: true, webdriver: false, hardwareConcurrency: 16, maxTouchPoints: 0, deviceMemory: 8,
...
const sandbox = {
  ...
  URL, URLSearchParams, TextEncoder, TextDecoder, crypto: require("crypto").webcrypto,
  Uint8Array, Uint16Array, Uint32Array, Int8Array, Int16Array, Int32Array, Float32Array, Float64Array,
  ArrayBuffer, DataView, BigInt, Symbol, Map, Set, WeakMap, WeakSet, Proxy, Reflect,
...
sandbox.navigator = trap("navigator", makeNavigator());
sandbox.location = trap("location", makeLocation());
sandbox.document = trap("document", {
  ...
  addEventListener: fakeFn, removeEventListener: fakeFn, readyState: "complete", cookie: "",
  referrer: "", title: "", visibilityState: "visible", hidden: false, onvisibilitychange: null,
});
sandbox.screen = trap("screen", { width: 1920, height: 1080, availWidth: 1920, availHeight: 1040, colorDepth: 24, pixelDepth: 24, orientation: { type: "landscape-primary" } });
...
const url = "https://www.shidianguji.com/api/ancientlib/read/reader-book/get/?bookId=NA11023&version=0&needCheckVersion=false&isWebpSupported=true&verifyFp=verify_abc&fp=abc&msToken=abc";
const xhr = new FakeXHR();
xhr.open("GET", url, true);
xhr.send(null);
const fu = new URL(xhr._finalUrl, url);
console.log("a_bogus len:", fu.searchParams.get("a_bogus").length);

console.log("--- accessed props ---");
console.log([...accessed].sort().join("\n"));
